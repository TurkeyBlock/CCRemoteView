...
class TurtleIdManager {
  ...
  save() {
    ...
    fs.writeFileSync(tmp, JSON.stringify({
      approved: [...this.approvedIds],
      allowByIp: this.allowByIp,
      // pending is not persisted — turtles re-request on reconnect
    }));
  ...
  load() {
    try {
      const data = JSON.parse(fs.readFileSync(this.idFile, 'utf8'));
...
module.exports = TurtleIdManager;
