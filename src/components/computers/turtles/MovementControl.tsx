...
export default function MovementControl({ computerId }: Props) {
  ...
  const cmdWithArgs = (command: string, args: (string | number | boolean | null | undefined)[]) => () => invokeCommand(computerId, command, args)
  ...
  return (
    <div className="group">
      ...
      <div className="btn-row-3">
        ...
        <button className="btn btn-compact" onClick={cmdWithArgs('drop', ['bottom'])}>Drop Dn</button>
        <button className="btn btn-compact" onClick={cmd('drop')}>Drop</button>
        <button className="btn btn-compact" onClick={cmdWithArgs('drop', ['top'])}>Drop Up</button>
      </div>
