...
export default function GlassesEditorLayout({ editor, canvasArea, onClose }: Props) {
  ...
  const sidebar = (
    <div style={{ width: 230, flexShrink: 0, borderLeft: '1px solid var(--line)', display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--surface)' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minHeight: 0 }}>
        <div style={{ background: 'var(--surface-3)', borderBottom: '1px solid var(--line)', padding: '5px 10px', fontSize: FS['12'], color: 'var(--fg-mute)', textTransform: 'uppercase', letterSpacing: 1, flexShrink: 0 }}>
          Properties
        </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', width: '100%', height: '100%', overflow: 'hidden' }}>
      ...
      {editorMode === 'draft' && (
        <div style={{ background: 'rgba(99,102,241,0.12)', borderBottom: '1px solid var(--line)', padding: '3px 12px', fontSize: FS['12'], color: 'var(--fg-mute)', display: 'flex', gap: 12, flexShrink: 0 }}>
